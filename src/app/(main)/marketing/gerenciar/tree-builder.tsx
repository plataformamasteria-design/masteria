  ...
  return (
    ...
    <div className="flex-1 w-full flex gap-4 lg:gap-6 h-full min-h-0 p-4 lg:p-6 bg-transparent">
      ...
      <div className="w-72 glass-card rounded-2xl flex flex-col py-4 overflow-y-auto flex-shrink-0 shadow-lg min-h-0">
        ...
        <div className="space-y-0.5">
          <div onClick={() => setActNode({ type: "campaign" })}
            className={`flex items-center gap-2 text-sm px-4 py-2 cursor-pointer transition-all border-l-2 relative ${!actNode || actNode.type === "campaign" ? "bg-primary/10 border-accent text-accent/90" : "border-transparent hover:bg-white/5 text-foreground/90"}`}>
          ...
          {tree.adsets.map((adset) => (
            <div key={adset.id} className="pt-0.5">
              <div 
                 ...
                 className={`flex items-center group relative cursor-pointer border-l-2 transition-all ${actNode?.type === "adset" && actNode.id === adset.id ? "bg-primary/10 border-accent text-accent/80" : "border-transparent text-foreground/90 hover:text-foreground hover:bg-white/5"}`}>
                <div className="relative z-10 w-full flex items-center px-4 pl-10 py-2 min-w-0">
                   ...
                   <div className="flex gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity ml-1">
                     <button onClick={(e) => { e.stopPropagation(); addAd(adset.id); }} title="Novo anúncio" className="h-5 w-5 flex items-center justify-center hover:bg-zinc-700/50 rounded text-foreground/90 hover:text-foreground"><Plus className="h-3 w-3" /></button>
                     <button onClick={(e) => { e.stopPropagation(); duplicateAdSet(adset.id); }} title="Duplicar" className="h-5 w-5 flex items-center justify-center hover:bg-zinc-700/50 rounded text-foreground/90 hover:text-accent/70"><Copy className="h-3 w-3" /></button>
              ...
              <div className="space-y-0.5 mt-0.5">
                {adset.ads.map((ad) => (
                  <div 
                    ...
                    className={`flex items-center group relative cursor-pointer transition-all border-l-2 ${actNode?.type === "ad" && actNode.id === ad.id ? "bg-primary/10 border-accent text-accent/90" : "border-transparent hover:bg-white/5 text-foreground/90 hover:text-foreground/90"}`}>
                    <div className="relative z-10 w-full flex items-center px-4 pl-16 py-1.5 min-w-0">
                      ...
                      <div className="flex gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity ml-1">
                        <button onClick={(e) => { e.stopPropagation(); duplicateAd(adset.id, ad.id); }} title="Duplicar" className="h-5 w-5 flex items-center justify-center hover:bg-zinc-700/50 rounded text-foreground/90 hover:text-accent/70"><Copy className="h-3 w-3" /></button>
      ...
      <div className="flex-1 glass-card rounded-2xl flex flex-col relative overflow-hidden shadow-2xl">
              ...
              <div className="flex flex-col gap-6 w-full">
                ...
                <div className="flex items-center justify-between pb-3 border-b border-white/5">
                   ...
                   <button
                     ...
                     className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border text-xs font-bold transition-all ${tree.start_active ? "border-primary/20 bg-primary/10 text-primary" : "border-white/10 bg-black/50 text-foreground/90 hover:text-foreground/90"}`}
                   >
                ...
                <div className="space-y-2">
                  ...
                  <Input value={tree.name} onChange={(e) => setTree({ ...tree, name: e.target.value })} className="bg-black/50 border-white/10 text-foreground rounded-xl h-11 focus-visible:ring-primary/50" />
                </div>
                <div className="grid grid-cols-1 gap-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={Array.isArray(tree.special_ad_categories) ? tree.special_ad_categories[0] : tree.special_ad_categories} onValueChange={(v) => setTree({ ...tree, special_ad_categories: [v] })}>
                      <SelectTrigger  className="bg-black/50 border-white/10 text-foreground rounded-xl h-11"><SelectValue placeholder="Selecione..." /></SelectTrigger>
                      <SelectContent>
                ...
                <div className="flex flex-col gap-4">
                  <div className="space-y-2">
                    ...
                    <Select value={tree.buying_type} onValueChange={(v: any) => setTree({ ...tree, buying_type: v })}>
                      <SelectTrigger  className="bg-black/50 border-white/10 text-foreground/90 rounded-xl h-11 pointer-events-none opacity-80"><SelectValue /></SelectTrigger>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                       ...
                       <Input type="number" placeholder="Opcional" value={tree.spend_cap} onChange={(e) => setTree({ ...tree, spend_cap: e.target.value })} className="bg-black/50 border-white/10 text-foreground rounded-xl pl-10 h-11 focus-visible:ring-primary/50" />
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-6 shadow-xl space-y-6 flex flex-col">
                <div className="flex items-center justify-between pb-3 border-b border-white/5">
                   <div>
                   ...
                   <button onClick={() => setTree({ ...tree, is_cbo: !tree.is_cbo })} className={`w-12 h-6 rounded-full transition-colors flex items-center p-1 ${tree.is_cbo ? "bg-accent" : "bg-zinc-700"}`}>
                  ...
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="flex flex-col gap-4 pt-1">
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                         ...
                         <Input type="number" value={tree.daily_budget} onChange={(e) => setTree({ ...tree, daily_budget: e.target.value })} className="bg-black/50 border-white/10 text-foreground rounded-xl font-mono text-base font-bold pl-10 h-11 focus-visible:ring-primary/50" />
                      </div>
                    </div>
                    <div className="space-y-2">
                      ...
                      <Select value={tree.bid_strategy} onValueChange={(v) => setTree({ ...tree, bid_strategy: v })}>
                        <SelectTrigger  className="bg-black/50 border-white/10 text-foreground rounded-xl h-11"><SelectValue /></SelectTrigger>
                        <SelectContent>
            ...
            return (
              <div className="w-full space-y-6">
                <div className="flex justify-between items-end pb-4 border-b border-white/5">
                  ...
                  <div className="flex gap-2">
                    <Button variant="outline" size="sm" onClick={() => duplicateAdSet(adset.id)} className="bg-zinc-900 border-white/10 text-primary hover:text-accent/70 hover:bg-zinc-800 rounded-xl"><Copy className="h-4 w-4 mr-2" /> Duplicar</Button>
                  </div>
                </div>
                <div className="flex flex-col gap-6 w-full">

              <div className="glass-card border border-white/5 rounded-2xl p-6 shadow-xl space-y-6">
                  <div className="space-y-2">
                     <Label  className="text-foreground/90 font-semibold text-xs">Nome de Identificação (Alias)</Label>
                     <Input value={adset.name} onChange={(e) => upd({ name: e.target.value })} className="bg-black/50 border-white/10 text-foreground rounded-xl h-11 text-base font-medium focus-visible:ring-primary/50" />
                  </div>
                </div>
              ...
              <div className="glass-card border border-white/5 rounded-2xl p-6 shadow-xl space-y-6">
                  ...
                  <div className="flex flex-col gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Select value={adset.optimization_goal || "__AUTO__"} onValueChange={(v) => upd({ optimization_goal: v === "__AUTO__" ? "" : v })}>
                        <SelectTrigger  className="bg-black/50 border-white/10 text-foreground rounded-xl h-11"><SelectValue placeholder="Automático pela Rede Neural" /></SelectTrigger>
                        <SelectContent>
                  ...
                  {adset.conversion_location === "MESSAGING_APP" && (
                    <div className="pt-5 border-t border-white/5 space-y-3 mt-4">
                      ...
                      <div className="flex flex-wrap gap-4">
                         <label className="flex items-center gap-2 text-sm text-foreground/90 hover:text-foreground transition-colors cursor-pointer select-none">
                           <input type="checkbox" 
                                  ...
                                  className="h-4 w-4 bg-black/50 border border-white/10 rounded text-primary focus:ring-primary focus:ring-offset-zinc-900" 
                           />
                           WhatsApp
                         </label>
                         <label className="flex items-center gap-2 text-sm text-foreground/90 hover:text-foreground transition-colors cursor-pointer select-none">
                  ...
                  <div className="flex flex-col gap-4 border-t border-white/5 pt-5 mt-2">
                    ...
                    <div className="space-y-2">
                       ...
                       <Select value={adset.custom_event_type} onValueChange={(v) => upd({ custom_event_type: v })}>
                         <SelectTrigger  className="bg-black/50 border-white/10 text-foreground rounded-xl h-11"><SelectValue placeholder="Evento" /></SelectTrigger>
                ...
                <div className="glass-card border border-white/5 rounded-2xl p-6 shadow-xl space-y-6">
                    ...
                    <div className="flex flex-col gap-4">
                      ...
                      <div className="space-y-2">
                         ...
                         <div className="relative">
                            <span className="absolute left-4 top-1/2 -translate-y-1/2 text-sm font-bold text-foreground/90">R$</span>
                            <Input type="number" value={adset.budget_type === "LIFETIME" ? adset.lifetime_budget : adset.daily_budget} onChange={(e) => adset.budget_type === "LIFETIME" ? upd({ lifetime_budget: e.target.value }) : upd({ daily_budget: e.target.value })} className="bg-black/50 border-white/10 text-foreground rounded-xl font-mono text-base font-bold pl-10 h-11 focus-visible:ring-rose-500/50" />
                         </div>
                      </div>
                    </div>
                    ...
                    <div className="border-t border-white/5 pt-4 flex flex-col gap-4">
                      ...
                      {(adset.bid_strategy === "LOWEST_COST_WITH_BID_CAP" || adset.bid_strategy === "COST_CAP") && (
                        <div className="space-y-2">
                          ...
                          <div className="relative">
                            <span className="absolute left-4 top-1/2 -translate-y-1/2 text-sm font-bold text-foreground/90">R$</span>
                            <Input type="number" value={adset.bid_amount} onChange={(e) => upd({ bid_amount: e.target.value })} placeholder="Ex: 5.00" className="bg-black/50 border-primary/20 text-foreground rounded-xl pl-10 h-11 focus-visible:ring-rose-500/50" />
                          </div>
                        </div>
                      )}
                      ...
                      {adset.bid_strategy === "MINIMUM_ROAS" && (
                        <div className="space-y-2">
                          ...
                          <Input type="number" step="0.1" value={adset.roas_average_floor || ""} onChange={(e) => upd({ roas_average_floor: e.target.value })} placeholder="Ex: 1.5" className="bg-black/50 border-primary/20 text-foreground rounded-xl h-11 focus-visible:ring-amber-500/50" />
                        </div>
                      )}
                    </div>
                  </div>
              ...
              <div className="glass-card border border-white/5 rounded-2xl p-6 shadow-xl space-y-6">
                   ...
                   <div className="flex flex-col gap-6">
                      <div className="space-y-2">
                         ...
                         <input type="datetime-local" value={adset.start_time} onChange={(e) => upd({ start_time: e.target.value })} className="flex h-11 w-full rounded-xl border border-white/10 bg-black/50 px-4 py-2 text-sm text-foreground focus-visible:outline-none focus:ring-1 focus:ring-primary/50" />
                      </div>
                      <div className="space-y-2">
                         <Label  className="text-foreground/90 font-semibold text-xs">Data de Encerramento (Opcional)</Label>
                         <input type="datetime-local" value={adset.end_time} onChange={(e) => upd({ end_time: e.target.value })} className="flex h-11 w-full rounded-xl border border-white/10 bg-black/50 px-4 py-2 text-sm text-foreground/90 focus-visible:outline-none focus:ring-1 focus:ring-white/10" />
                      </div>
                   </div>
                </div>
                ...
                <div className="glass-card border border-white/5 rounded-2xl p-6 shadow-xl space-y-6">
                     ...
                     <div className="flex flex-col gap-4">
                       <div className="space-y-2">
                          ...
                          <Select value={adset.age_min} onValueChange={(v) => upd({ age_min: v })}><SelectTrigger  className="bg-black/50 border-white/10 text-foreground rounded-xl h-11"><SelectValue /></SelectTrigger><SelectContent>{Array.from({ length: 48 }, (_, i) => i + 18).map((n) => <SelectItem key={n} value={String(n)}>{n}</SelectItem>)}</SelectContent></Select>
                       </div>
                       <div className="space-y-2">
                          <Label  className="text-foreground/90 font-semibold text-xs">Idade (Upper Limit)</Label>
                          <Select value={adset.age_max} onValueChange={(v) => upd({ age_max: v })}><SelectTrigger  className="bg-black/50 border-white/10 text-foreground rounded-xl h-11"><SelectValue /></SelectTrigger><SelectContent>{Array.from({ length: 48 }, (_, i) => i + 18).map((n) => <SelectItem key={n} value={String(n)}>{n === 65 ? "65+" : n}</SelectItem>)}</SelectContent></Select>
                       </div>
                       <div className="space-y-2">
                          <Label  className="text-foreground/90 font-semibold text-xs">Gênero</Label>
                          <Select value={adset.genders} onValueChange={(v) => upd({ genders: v })}><SelectTrigger  className="bg-black/50 border-white/10 text-foreground rounded-xl h-11"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="ALL">Todo Espectro Livre</SelectItem><SelectItem value="MALE">Apenas Homens (M)</SelectItem><SelectItem value="FEMALE">Apenas Mulheres (F)</SelectItem></SelectContent></Select>
                       </div>
                     </div>
                  </div>
                )}
                  ...
                  <div className="flex items-center justify-between pb-3 border-b border-white/5">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <button onClick={() => upd({ placements_type: adset.placements_type === "ADVANTAGE" ? "MANUAL" : "ADVANTAGE" })}
                        className={`w-10 h-5 rounded-full transition-colors flex items-center p-0.5 ${adset.placements_type === "ADVANTAGE" ? "bg-accent" : "bg-zinc-700"}`}>
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex flex-wrap gap-3">
                          {[
                            ...
                            return (
                              <label key={plt.val} className={`flex items-center gap-2 px-3 py-2 rounded-xl border text-sm cursor-pointer transition-all ${checked ? "border-accent/60 bg-primary/10 text-accent/70" : "border-white/10 text-foreground/90 hover:border-white/20"}`}>
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {[
                              ...
                              return (
                                <label key={pos.val} className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border text-xs cursor-pointer transition-all ${checked ? "border-accent/40 bg-primary/10 text-accent/70" : "border-white/10 text-foreground/90 hover:border-white/20"}`}>
                                  <input type="checkbox" className="hidden" checked={!!checked}
                                    onChange={(e) => {
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {[
                              ...
                              return (
                                <label key={pos.val} className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border text-xs cursor-pointer transition-all ${checked ? "border-primary/20 bg-primary/10 text-primary" : "border-white/10 text-foreground/90 hover:border-white/20"}`}>
                                  <input type="checkbox" className="hidden" checked={!!checked}
                                    onChange={(e) => {
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex gap-3">
                          {[{ val: "mobile", label: "Mobile" }, { val: "desktop", label: "Desktop" }].map((dev) => {
                            ...
                            return (
                              <label key={dev.val} className={`flex items-center gap-2 px-3 py-2 rounded-xl border text-sm cursor-pointer transition-all ${checked ? "border-primary/20 bg-primary/10 text-primary" : "border-white/10 text-foreground/90 hover:border-white/20"}`}>
                                <input type="checkbox" className="hidden" checked={!!checked}
                                  onChange={(e) => {
          ...
          {actNode?.type === "ad" && activeAdContext && (
            <div className="w-full space-y-6">
              <div className="flex justify-between items-end pb-4 border-b border-white/5">
                ...
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" onClick={() => duplicateAd(activeAdContext.adset.id, activeAdContext.ad.id)} className="bg-zinc-900 border-white/10 text-primary hover:text-accent/70 hover:bg-zinc-800 rounded-xl"><Copy className="h-4 w-4 mr-2" /> Clonar Ad</Button>
                </div>
              </div>
              ...
              <div className="glass-card border border-primary/20 rounded-2xl p-6 shadow-xl space-y-6 relative overflow-hidden">
                ...
                <div className="space-y-2 relative z-10">
                   ...
                   <Input value={activeAdContext.ad.name} onChange={(e) => updateAd(activeAdContext.adset.id, activeAdContext.ad.id, { name: e.target.value })} className="bg-black/60 border-white/10 text-foreground rounded-xl h-11 text-base font-medium focus-visible:ring-primary/50 my-1" placeholder="Ex: AD 01 - Img Promo..." />
                </div>
              </div>
        ...
        <div className="absolute bottom-0 inset-x-0 p-4 border-t border-white/5 bg-black/80 backdrop-blur-3xl z-40">
           <div className="max-w-3xl mx-auto flex items-center justify-between gap-4">
              ...
              <div className="flex gap-3 ml-auto">
                ...
                }} variant="outline" className="border-white/10 bg-zinc-800 hover:bg-zinc-700 text-foreground/90 rounded-xl h-12 font-bold px-6">
      ...
      {showValidationModal && (
        <div className="fixed inset-0 z-[200] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-zinc-900 border border-white/10 rounded-2xl w-full max-w-2xl flex flex-col max-h-[85vh] shadow-2xl overflow-hidden">
               ...
               {validationErrs.length === 0 ? (
                 ...
                 validationErrs.map((err, i) => (
                   <div key={i} className={`p-4 rounded-xl border flex items-start gap-3 ${err.level === 'error' ? 'bg-primary/10 border-primary/20' : 'bg-primary/10 border-primary/20'}`}>
                     ...
                     <Button  size="sm" variant="ghost" className="h-8 hover:bg-white/5 text-foreground/90 hover:text-foreground" onClick={() => { jumpToError(err.nodeId); setShowValidationModal(false); }}>Ir ao campo</Button>
                   </div>
                 ))
               )}
            </div>
            <div className="p-5 border-t border-white/5 bg-black/40 flex justify-end">
               <Button onClick={() => setShowValidationModal(false)} className="bg-zinc-800 hover:bg-zinc-700 text-foreground font-bold h-11 px-8 rounded-xl">Fechar</Button>
            </div>
          </div>
        </div>
      )}
