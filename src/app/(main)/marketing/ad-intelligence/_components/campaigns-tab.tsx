           ...
           <tbody className="divide-y divide-white/5 text-zinc-300 font-medium">
             {campaigns.map((c: any) => (
               <tr key={c.id} className="hover:bg-white/[0.02] transition-colors">
                 ...
                 <td className={cn("px-4 py-4 text-right", (c.cpl || 0) > 40 ? "text-red-400 font-bold" : "text-emerald-400")}>R$ {(c.cpl || 0).toFixed(2)}</td>
                 <td className="px-4 py-4 text-right">{(c.ctr || 0).toFixed(2)}%</td>
                 <td className="px-4 py-4 text-right">
                   {c.hookRate ? (
                     <span className={cn("px-2 py-0.5 rounded text-[11px] font-bold", c.hookRate >= 25 ? "bg-emerald-500/10 text-emerald-400" : c.hookRate >= 15 ? "bg-amber-500/10 text-amber-400" : "bg-red-500/10 text-red-400")}>
                 ...
                 <td className="px-4 py-4 text-center">
                    <span className={cn("px-2 py-1 flex items-center justify-center gap-1 mx-auto w-12 rounded-lg text-xs font-black", 
                        (c.score || 50) >= 80 ? "bg-emerald-500/20 text-emerald-400 border border-emerald-500/30" : 
                        (c.score || 50) <= 50 ? "bg-red-500/20 text-red-400 border border-red-500/30" : 
                 ...
                 <td className="px-4 py-4 min-w-[300px]">
                   <div className="flex items-start gap-2 text-xs">
                     <Cpu className={cn("h-4 w-4 shrink-0 mt-0.5", c.score >= 80 ? "text-emerald-400" : "text-amber-400")} />
                     <div className="flex flex-col gap-2">
                       ...
                       <button 
                         ...
                         className="flex items-center gap-1.5 w-fit px-2 py-1 bg-white/5 hover:bg-white/10 text-zinc-300 rounded text-[10px] uppercase font-bold tracking-widest border border-white/5 transition-colors"
                       >
