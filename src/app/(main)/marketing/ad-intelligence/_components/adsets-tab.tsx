             ...
             {adsets.map((c: any) => (
               <tr key={c.id} className="hover:bg-white/[0.02] transition-colors">
                 <td className="px-4 py-4 max-w-[250px] truncate flex items-center gap-2">
                   <div className={cn("w-2 h-2 rounded-full", c.status === "ACTIVE" ? "bg-emerald-500" : "bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]")} />
                   {c.name}
                 </td>
                 ...
                 <td className="px-4 py-4 text-center">
                   <span className={cn("px-2 py-1 flex items-center justify-center gap-1 mx-auto w-12 rounded-lg text-xs font-black bg-zinc-800 text-zinc-300 border border-white/10")}>
                      {c.score || 50}
                   </span>
                 </td>
               </tr>
