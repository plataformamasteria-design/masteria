/**
 * POST /api/meta/ativar — Ativa anúncio/conjunto/campanha via Meta API.
...
export async function POST(req: NextRequest) {
  try {
    ...
    const res = await fetch(`${META_BASE}/${objeto_id}`, {
      ...
      body: JSON.stringify({ status: "ACTIVE", access_token: auth.token }),
    });
    ...
    return NextResponse.json({ success: true, message: `${tipoLabel} ${objeto_id} ativado com sucesso.` });
  } catch (e: any) {
    console.error("[api/meta/ativar]", e);
