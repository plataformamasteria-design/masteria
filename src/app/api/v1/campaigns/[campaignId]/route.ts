...
export async function GET(request: NextRequest, { params }: { params: Promise<{ campaignId: string }> }) {
    try {
        ...
        const failedWhatsappSubquery = db.select({ value: sql<number>`count(*)::int` }).from(whatsappDeliveryReports).where(and(eq(whatsappDeliveryReports.campaignId, campaignId), eq(whatsappDeliveryReports.companyId, companyId), eq(whatsappDeliveryReports.status, 'failed')));
