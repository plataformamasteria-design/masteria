...
export const AddTaskNodeV4 = memo(({ data, selected }: NodePropsV4) => {
    ...
    return (
        ...
        >
            ...
            <div className="bg-zinc-50 dark:bg-zinc-900/50 rounded-xl border border-zinc-100 dark:border-zinc-800/80 px-3 py-2.5 space-y-2">
                {taskText ? (
                    <p className="text-[11px] text-zinc-700 dark:text-zinc-200 line-clamp-2 leading-relaxed">{taskText}</p>
                ) : (
                    <p className="text-[11px] text-zinc-400 dark:text-zinc-400 italic">Configurar tarefa...</p>
                )}
                
                {(dueDate || assignee) && (
                    <div className="flex items-center gap-2 pt-1 border-t border-zinc-100/60">
                        {dueDate && (
                            <span className="text-[9px] font-medium text-zinc-500 dark:text-zinc-400 bg-zinc-200/50 dark:bg-zinc-800/50 px-1.5 py-0.5 rounded">
                                ⏱️ {dueDate}
                            </span>
                        )}
                        {assignee && (
                            <span className="text-[9px] font-medium text-zinc-500 dark:text-zinc-400 bg-zinc-200/50 dark:bg-zinc-800/50 px-1.5 py-0.5 rounded truncate max-w-[80px]">
                                👤 {assignee}
                            </span>
                        )}
                    </div>
