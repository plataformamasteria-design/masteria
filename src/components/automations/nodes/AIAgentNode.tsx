...
import { getSystemGlobalAiStatus } from "@/app/actions/automations-builder";
                ...
                return (
                  <CommandItem
                    ...
                    onSelect={() => {
                      ...
                      const content = ((Array.isArray(cleanFields) ? cleanFields : []) || []).map((f: any) => `${f.key}:\n${f.value}`).join('\n\n');
                      onSelect(content);
                      setOpen(false);
                    }}
...
function AIAgentNodeComponent({ id, data }: NodeProps) {
  ...
  useEffect(() => {
    ...
    setCredentials([]);
    getSystemGlobalAiStatus().then(status => {
  ...
  const updateTool = (index: number, field: string, value: string) => {
    const updated = ((Array.isArray(tools) ? tools : []) || []).map((t, i) => (i === index ? { ...t, [field]: value } : t));
  ...
  return (
    <div className="bg-white dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.2)] min-w-[320px] max-w-[420px] overflow-visible transition-all hover:shadow-xl group">
      ...
      <div className="px-3 py-1.5 flex items-center gap-2 border-b border-slate-200 dark:border-zinc-800/40">
      ...
      <div className="px-3 pb-3 pt-2 max-h-[600px] overflow-y-auto nowheel nodrag">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <TabsContent value="agent" className="space-y-3 mt-0">
            ...
            {!headPromptName && (
              <div className="rounded-md bg-muted/40 border border-slate-200 dark:border-zinc-800/50 px-2 py-1.5">
          ...
          <TabsContent value="model" className="space-y-3 mt-0">
            <div className="space-y-1">
              <Label className="text-[10px] font-semibold text-muted-foreground uppercase">Conexão WhatsApp API</Label>
              <Select value={connectionId || "default"} onValueChange={(v) => updateConfig({ connection_id: v === "default" ? "" : v })}>
                <SelectTrigger className="h-7 text-[11px] nodrag"><SelectValue placeholder="Usar conexão ativa padrão" /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="default" className="text-[11px] text-muted-foreground">Usar conexão ativa padrão</SelectItem>
                  {whatsappConnections.map((conn) => (
                    <SelectItem key={conn.id} value={conn.id} className="text-[11px]">
                      {conn.config_name || conn.configName || conn.id.slice(0,8)} ({conn.connection_type || conn.connectionType})
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
            ...
            {showNewCred && (
              <div className="space-y-2 rounded-lg bg-muted/30 p-2 border border-slate-200 dark:border-zinc-800/50">
          ...
          <TabsContent value="tools" className="space-y-3 mt-0">
            ...
            {((Array.isArray(tools) ? tools : []) || []).map((tool, i) => (
              <div key={i} className="space-y-2 rounded-lg bg-muted/30 p-2 border border-slate-200 dark:border-zinc-800/50">
                <div className="flex items-center justify-between">
      ...
      <div className="flex justify-around px-4 pb-4 pt-1">
        <div className="flex flex-col items-center gap-0.5">
          <span className="text-[8px] font-bold text-emerald-600 dark:text-emerald-400 tracking-wider uppercase mb-1">Concluído</span>
        </div>
        <div className="flex flex-col items-center gap-0.5">
