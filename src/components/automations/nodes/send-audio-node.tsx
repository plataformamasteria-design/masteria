...
export const SendAudioNode = memo(({ data, selected }: any) => {
    return (
        <div
            className={`px-5 py-4 shadow-[0_4px_24px_rgba(0,0,0,0.06)] rounded-2xl bg-white border ${selected ? 'border-green-500' : 'border-green-500/20'} min-w-[280px] max-w-[320px]  animate-in zoom-in duration-200 group/node relative transition-all`}
        >
            <Handle
                type="target"
                ...
                {data.file_url ? (
                    <div className="flex items-center gap-2">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[10px] text-gray-400 mt-1 truncate">{data.file_url}</p>
                        </div>
                    </div>
                ) : (
