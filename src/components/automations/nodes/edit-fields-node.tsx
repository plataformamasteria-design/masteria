...
export const EditFieldsNode = memo(({ data, selected }: any) => {
    ...
    return (
        <div
            className={`px-5 py-4 shadow-[0_4px_24px_rgba(0,0,0,0.06)] rounded-2xl bg-white border ${selected ? 'border-teal-500' : 'border-teal-500/20'} min-w-[280px] max-w-[340px]  animate-in zoom-in duration-200 group/node relative transition-all`}
        >
            <Handle
                type="target"
                ...
                {mode === 'pairs' && fields.length > 0 ? (
                    ((Array.isArray(fields) ? fields : []) || []).map((field, i) => (
                        <div key={i} className="flex items-center gap-1.5 text-[11px]">
                            <span className="font-mono text-teal-600 bg-teal-50 px-1.5 py-0.5 rounded truncate max-w-[80px]">
                                {field.name || '?'}
                            </span>
                            <span className="text-gray-400">=</span>
                            <span className="text-gray-500 truncate max-w-[100px]">{field.value || '...'}</span>
                        </div>
                    ))
                ) : mode === 'json' ? (
                    <pre className="text-[10px] text-teal-700 font-mono line-clamp-3 whitespace-pre-wrap">
                        {data.json_value || '{}'}
                    </pre>
                ) : (
