...
const bgColors = {
    capture: 'bg-fuchsia-500/10',
    wait_response: 'bg-violet-500/10',
};
    ...
    return (
        <div className={`px-5 py-4 shadow-[0_4px_24px_rgba(0,0,0,0.06)] rounded-2xl bg-white border ${selected ? 'border-primary' : borderColors[type]} min-w-[240px]  animate-in zoom-in duration-200 group/node relative transition-all`}>
            ...
            <div className="space-y-3">
                <div className="text-[10px] text-gray-400 font-black uppercase tracking-widest pl-1 opacity-60">
                    {type === 'capture' ? 'Armazenar em' : 'Condição de Retomada'}
                </div>
                <div className="text-sm text-gray-700 font-mono bg-gray-50/80 p-3 rounded-xl border border-gray-100 flex items-center justify-between group">
                    {type === 'capture' ? (
                        <>
                            <span className="font-bold">{data.field || 'selecionar_campo'}</span>
                            <span className="text-[10px] text-fuchsia-600 underline cursor-pointer opacity-0 group-hover:opacity-100 transition-opacity">Editar</span>
                        </>
                    ) : (
                        <div className="flex items-center gap-2">
                            <span className="w-1.5 h-1.5 rounded-full bg-violet-600 animate-pulse shadow-[0_0_8px_rgba(124,58,237,0.5)]" />
                            <span className="text-xs font-semibold text-gray-500">Próxima interação...</span>
                        </div>
                    )}
                </div>
            </div>
