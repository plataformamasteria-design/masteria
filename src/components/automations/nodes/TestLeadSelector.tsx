...
import { getTestLeadsForSelector } from "@/app/actions/automations-builder";
...
export function TestLeadSelector({ value, onChange }: TestLeadSelectorProps) {
  ...
  const searchLeads = useCallback(async (q: string) => {
    ...
    setLoading(true);
    try {
      const leadsData = await getTestLeadsForSelector(q);
      if (leadsData) {
        setResults(
          (leadsData || []).map((c: any) => ({
            id: c.id,
  ...
  return (
    <div className="space-y-1">
      <button
        className="w-full flex items-center gap-2 text-left rounded-lg border border-dashed border-slate-200 dark:border-zinc-800 px-2 py-1.5 hover:bg-accent/30 transition-colors"
      ...
      {open && (
        <div className="rounded-lg border border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 shadow-md overflow-visible">
          <div className="p-1.5">
            <Input
              value={search}
          ...
          <div className="max-h-[160px] overflow-y-auto">
            ...
            {((Array.isArray(results) ? results : []) || []).map((lead) => (
              <button
                key={lead.id}
