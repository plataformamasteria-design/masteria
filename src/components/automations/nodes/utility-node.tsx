...
export const UtilityNode = memo(({ data, selected }: any) => {
    ...
    return (
        <div className={`px-5 py-4 shadow-[0_8px_30_px_rgb(0,0,0,0.04)] rounded-2xl bg-white border ${selected ? 'border-amber-600' : 'border-amber-500/20'} min-w-[260px]  animate-in zoom-in duration-200 group/node relative transition-all`}>
            {/* Botão de Excluir */}
            <button
                onClick={(e) => {
                    e.stopPropagation();
            ...
            <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                    <span className="block text-[10px] font-black text-amber-600 opacity-70 uppercase tracking-[0.2em]">Utilitários de Dados</span>
                    <span className="text-sm font-bold text-slate-900 tracking-tight">{utilityLabels[type]}</span>
                </div>
            </div>
