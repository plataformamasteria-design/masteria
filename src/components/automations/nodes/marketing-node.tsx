...
export const MarketingNode = memo(({ data, selected }: any) => {
    ...
    return (
        <div className={`px-5 py-4 shadow-[0_4px_24px_rgba(0,0,0,0.06)] rounded-2xl bg-white dark:bg-zinc-900 border ${selected ? 'border-orange-600' : 'border-orange-500/20'} min-w-[260px]  animate-in zoom-in duration-200 group/node relative transition-all`}>
            {/* Botão de Excluir */}
            <button
                onClick={(e) => {
                    e.stopPropagation();
            ...
            <div className="flex items-center gap-3 mb-4">
                <div className="p-2 bg-orange-50 dark:bg-orange-900/30 rounded-xl shadow-inner">
                    <Icon className="h-5 w-5 text-orange-600 dark:text-orange-400" />
                </div>
                <div>
                    <span className="block text-[10px] font-black text-orange-600 dark:text-orange-400 opacity-70 uppercase tracking-[0.2em]">Marketing & Envios</span>
                    <span className="text-sm font-bold text-slate-900 dark:text-zinc-100 tracking-tight">{marketingLabels[type]}</span>
                </div>
            </div>
