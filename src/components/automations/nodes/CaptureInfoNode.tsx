...
function CaptureInfoNodeComponent({ id, data }: NodeProps) {
  ...
  return (
    <div className="bg-white dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.2)] min-w-[280px] overflow-visible group">
      ...
      <div className="px-4 py-3 space-y-3">
        <div>
          ...
          <Select value={fieldKey} onValueChange={(v) => { setFieldKey(v); updateConfig({ field_key: v }); }}>
            ...
            <SelectContent>{((Array.isArray(fields) ? fields : []) || []).map((f) => (<SelectItem key={f.id} value={f.field_key}><div className="flex items-center gap-2"><span className={`h-1.5 w-1.5 rounded-full shrink-0 ${f.is_system ? 'bg-violet-50 dark:bg-violet-900/300' : 'bg-emerald-50 dark:bg-emerald-900/300'}`} />{f.field_label}</div></SelectItem>))}</SelectContent>
          </Select>
        </div>
        <div>
      ...
      <Handle type="source" position={Position.Bottom} className="!w-4 !h-4 !bg-violet-50 dark:bg-violet-900/300 !border-[3px] !border-white dark:border-zinc-900 dark:!border-zinc-950 shadow-sm z-50 transition-transform hover:scale-125 px] dark:!border-zinc-950 shadow-sm z-50 transition-transform hover:scale-125" />
    </div>
  );
}
