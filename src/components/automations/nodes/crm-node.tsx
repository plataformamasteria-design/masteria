...
export const CRMNode = memo(({ data, selected }: any) => {
    ...
    return (
        <div className={`px-5 py-4 shadow-[0_4px_24px_rgba(0,0,0,0.06)] rounded-2xl bg-white border ${selected ? 'border-emerald-600' : 'border-emerald-500/20'} min-w-[260px]  animate-in zoom-in duration-200 group/node relative transition-all`}>
            {/* Botão de Excluir */}
            <button
                onClick={(e) => {
                    e.stopPropagation();
            ...
            <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                    <span className="block text-[10px] font-black text-emerald-600 opacity-70 uppercase tracking-[0.2em]">CRM & Kanban</span>
                    <span className="text-sm font-bold text-slate-900 tracking-tight">{crmLabels[type]}</span>
                </div>
            </div>
