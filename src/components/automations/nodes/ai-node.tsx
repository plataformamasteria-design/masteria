...
export const AINode = memo(({ data, selected }: any) => {
    ...
    return (
        <div className={`px-5 py-4 shadow-[0_4px_24px_rgba(0,0,0,0.06)] rounded-2xl bg-white border ${selected ? 'border-indigo-600' : 'border-indigo-500/20'} min-w-[260px] animate-in zoom-in duration-200 group/node relative transition-all`}>
            {/* Botão de Excluir */}
            <button
            ...
            <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                    <span className="block text-[10px] font-black text-indigo-600 opacity-70 uppercase tracking-[0.2em]">Agente de IA</span>
                    <span className="text-sm font-bold text-slate-900 tracking-tight">{data.label || aiLabels[type] || 'AI Agent'}</span>
                </div>
            </div>

            {/* Config summary */}
            <div className="text-sm font-semibold text-gray-700 bg-gray-50/80 p-3 rounded-xl border border-gray-100 space-y-1.5">
                <div className="flex items-center justify-between">
                    <span className="text-[10px] font-bold text-indigo-500 uppercase">✨ {data.provider?.toUpperCase() || 'GEMINI'}</span>
                    <span className="text-[10px] text-gray-400 font-mono">{data.model || 'gemini-2.0-flash'}</span>
                </div>
                {isDialogue && (
                    <div className="flex items-center gap-1.5">
                        <span className="w-1.5 h-1.5 rounded-full bg-violet-500 animate-pulse" />
                        <span className="text-[10px] text-violet-600 font-bold">Modo Diálogo</span>
                        {data.max_turns && (
                            <span className="text-[10px] text-gray-400 ml-auto">{data.max_turns} turnos</span>
                        )}
                    </div>
                )}
            ...
            {isDialogue ? (
                <>
                    <div className="flex justify-between mt-3 text-[9px] font-bold tracking-wider px-1">
                        ...
                        <span className="text-amber-600">⚠ MAX TURNOS</span>
                    </div>
                    <Handle
                        type="source"
                        position={Position.Bottom}
                        id="completed"
                        className="!w-4 !h-4 !bg-white !border-[3px] !border-white dark:!border-zinc-950 shadow-sm z-50 transition-transform hover:scale-125 px] shadow-md !-bottom-[10px] !left-[30%] z-50 transition-transform hover:scale-125"
                    />
                    <Handle
                        type="source"
                        position={Position.Bottom}
                        id="max_turns"
                        className="!w-4 !h-4 !bg-white !border-[3px] !border-white dark:!border-zinc-950 shadow-sm z-50 transition-transform hover:scale-125 px] shadow-md !-bottom-[10px] !left-[70%] z-50 transition-transform hover:scale-125"
                    />
                </>
            ) : (
                <>
