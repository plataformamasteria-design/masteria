...
function EditFieldsNodeComponent({ id, data }: NodeProps) {
  ...
  const updateField = (fieldId: string, key: keyof FieldMapping, val: string) =>
    updateConfig({ fields: ((Array.isArray(fields) ? fields : []) || []).map((f) => (f.id === fieldId ? { ...f, [key]: val } : f)) });
  ...
  const handleDropOnField = (e: React.DragEvent, fieldId: string, targetKey: "name" | "value") => {
    ...
    if (outputData) {
      try {
        ...
        const updatedFields = ((Array.isArray(fields) ? fields : []) || []).map((f) => {
        ...
        <div className="px-3 pb-3 space-y-3 border-t border-slate-200 dark:border-zinc-800/40 pt-3 max-h-[500px] overflow-y-auto nowheel nodrag">
          <div className="space-y-1">
            ...
            <Select value={mode} onValueChange={(v) => updateConfig({ mode: v })}>
              ...
              <SelectContent>{((Array.isArray(MODES) ? MODES : []) || []).map((m) => (<SelectItem key={m.value} value={m.value} className="text-[11px]">{m.label}</SelectItem>))}</SelectContent>
            </Select>
          </div>
          <div className="space-y-1">
          ...
          {mode === "manual" ? (
            <div className="space-y-2">
              ...
              {((Array.isArray(fields) ? fields : []) || []).map((field) => (
                <div key={field.id} className="rounded-lg border border-slate-200 dark:border-zinc-800/60 p-2 space-y-1.5 bg-muted/30">
                  ...
                  <Select value={field.type} onValueChange={(v) => updateField(field.id, "type", v)}>
                    ...
                    <SelectContent>{((Array.isArray(FIELD_TYPES) ? FIELD_TYPES : []) || []).map((t) => (<SelectItem key={t.value} value={t.value} className="text-[11px]">{t.label}</SelectItem>))}</SelectContent>
                  </Select>
                  <div className="flex items-center gap-1">
