...
export const SystemNode = memo(({ data, selected }: any) => {
    ...
    return (
        <div className={`px-5 py-4 shadow-[0_4px_24px_rgba(0,0,0,0.06)] rounded-2xl bg-white border ${selected ? 'border-slate-600' : 'border-slate-500/20'} min-w-[260px]  animate-in zoom-in duration-200 group/node relative transition-all`}>
            {/* Botão de Excluir */}
            <button
                onClick={(e) => {
                    e.stopPropagation();
            ...
            <div className="flex items-center gap-3 mb-4">
                <div className="p-2 bg-slate-500/10 rounded-xl shadow-inner">
                    <Icon className="h-5 w-5 text-gray-500" />
                </div>
                <div>
                    <span className="block text-[10px] font-black text-gray-500 opacity-70 uppercase tracking-[0.2em]">Integração & Sistema</span>
                    <span className="text-sm font-bold text-slate-900 tracking-tight">{systemLabels[type]}</span>
                </div>
            </div>
