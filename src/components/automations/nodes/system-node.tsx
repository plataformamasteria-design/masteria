...
import { Globe, Bell, BotOff, Terminal, X } from 'lucide-react';
...
const systemLabels = {
    webhook: 'Webhook Externo (HTTP)',
    notify: 'Notificar Equipe',
    pause: 'Pausar Atendimento Bot',
    code: 'Executar Script',
};

export const SystemNode = memo(({ data, selected }: any) => {
    const type = data.systemType as keyof typeof systemIcons || 'webhook';
    const Icon = systemIcons[type];

    return (
        <div className={`px-5 py-4 shadow-[0_4px_24px_rgba(0,0,0,0.06)] rounded-2xl bg-white dark:bg-zinc-900 border ${selected ? 'border-slate-600' : 'border-slate-500/20'} min-w-[260px]  animate-in zoom-in duration-200 group/node relative transition-all`}>
            {/* Botão de Excluir */}
            <button
                onClick={(e) => {
                    e.stopPropagation();
            ...
            <div className="flex items-center gap-3 mb-4">
                <div className="p-2 bg-slate-50 dark:bg-zinc-900/50 rounded-xl shadow-inner">
                    <Icon className="h-5 w-5 text-gray-500 dark:text-zinc-400" />
                </div>
                <div>
                    <span className="block text-[10px] font-black text-gray-500 dark:text-zinc-400 opacity-70 uppercase tracking-[0.2em]">Integração & Sistema</span>
                    <span className="text-sm font-bold text-slate-900 dark:text-zinc-100 tracking-tight">{systemLabels[type]}</span>
                </div>
            </div>
            ...
            <div className="text-sm font-semibold text-gray-700 dark:text-zinc-200 bg-gray-50/80 p-3 rounded-xl border border-gray-100 dark:border-zinc-800/80">
                {data.description || 'Configurar parâmetros do sistema...'}
            </div>
