...
const PendingAuthorization = () => {
  ...
  const { isImpersonating, stopImpersonating } = useUserRole();
  ...
  return (
    <div className="relative min-h-screen flex flex-col bg-background selection:bg-primary/20 overflow-hidden text-foreground">
      {isImpersonating && (
        <div className="bg-amber-500 text-amber-950 px-4 py-2 flex items-center justify-between text-sm font-bold z-50 shadow-md w-full">
          <div className="flex items-center gap-2">
            <span className="animate-pulse">⚠️</span>
            <span>Você está operando como outro usuário (Identidade Assumida).</span>
          </div>
          <Button size="sm" className="bg-amber-950 text-amber-50 hover:bg-amber-900 border-none h-7 text-xs" onClick={stopImpersonating}>
            Voltar para Admin
          </Button>
        </div>
      )}
