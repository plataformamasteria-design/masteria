...
import { User, Mail, Lock, Image as ImageIcon, Moon, Sun, Volume2, VolumeX, PenLine, Users, Palette, Building2, Settings, Eye, EyeOff, Globe, Sparkles } from "lucide-react";
...
import { Facebook, Chrome, Link2, LayoutGrid, BarChart3, MessageCircle, Smartphone } from "lucide-react";
...
const Profile = () => {
  ...
  const fetchProfile = async () => {
    try {
      ...
      if (profileData) {
        ...
        setAiReplyEnabled(profileData.ai_reply_suggestion_enabled ?? false);
        // Load saved language
  ...
  const updateAiReplySuggestion = async (enabled: boolean) => {
    try {
      if (!profile) return;
      setAiReplyEnabled(enabled);
      const { error } = await (supabase.from('profiles') as any)
        .update({ ai_reply_suggestion_enabled: enabled })
        .eq('id', profile.id);
      if (error) throw error;
      toast({
        title: 'Preferência atualizada',
        description: enabled
          ? 'Botão de sugestão com I.A ativado no chat.'
          : 'Botão de sugestão com I.A desativado.',
      });
    } catch (error: any) {
      console.error('Error updating ai_reply_suggestion_enabled:', error);
      toast({ title: 'Erro ao atualizar preferência', description: error.message, variant: 'destructive' });
      await fetchProfile();
    }
  };
  ...
  const updateMultiAccountEnabled = (enabled: boolean) => {
    setMultiAccountEnabled(enabled);
    localStorage.setItem('vitta_multi_account_enabled', enabled ? 'true' : 'false');
    window.dispatchEvent(new Event('multi_account_changed'));
    toast({
      title: 'Preferência atualizada',
      description: enabled
        ? 'Troca Rápida de Múltiplas Contas ativada.'
        : 'Troca Rápida de Múltiplas Contas desativada.',
    });
  };
  ...
  if (canManageOrg) visibleTabs.push('conexoes');
  if (canManageOrg) visibleTabs.push('integracoes');
  if (canManageUsers) visibleTabs.push('usuarios');
  visibleTabs.push('cores');
  const tabCount = visibleTabs.length;
  const gridClass = tabCount <= 2 ? 'grid-cols-2' : tabCount === 3 ? 'grid-cols-3' : tabCount === 4 ? 'grid-cols-4' : tabCount === 5 ? 'grid-cols-5' : 'grid-cols-6';
            ...
            <TabsContent value="profile" className="space-y-6 md:space-y-8 mt-6">
              ...
              <Card className="border-0 shadow-xl bg-card/80 backdrop-blur-sm hover:shadow-2xl transition-all">
                ...
                <CardContent className="space-y-4">
                  ...
                  {/* AI Reply Suggestion toggle */}
                  <div className="p-4 rounded-xl bg-gradient-to-br from-violet-500/5 to-background/30 hover:from-violet-500/10 hover:to-background/40 transition-all border border-violet-500/10">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        <div className="p-3 bg-gradient-to-br from-violet-500/20 to-violet-500/10 rounded-lg">
                          <svg
                        ...
                        <div>
                          <p className="font-medium">Sugestão de Resposta com I.A</p>
                          <p className="text-sm text-muted-foreground">
                            {aiReplyEnabled
                              ? 'Botão ✨ ativo no chat — gera sugestões sem enviar'
                              : 'Ativar botão ✨ no chat para gerar respostas com I.A'}
                          </p>
                        </div>
                      </div>
                      <Switch
                        checked={aiReplyEnabled}
                        onCheckedChange={updateAiReplySuggestion}
                  ...
                  <div className="p-4 rounded-xl bg-gradient-to-br from-background/50 to-background/30 hover:from-background/60 hover:to-background/40 transition-all">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="font-medium">Múltiplas Contas (Botão Flutuante)</p>
                          <p className="text-sm text-muted-foreground">
                            {multiAccountEnabled
                              ? "Ativado: O botão flutuante de contas ficará visível."
                              : "Desativado: O botão flutuante ficará oculto."}
                          </p>
                        </div>
                      </div>
                      <Switch
                        checked={multiAccountEnabled}
                        onCheckedChange={updateMultiAccountEnabled}
                        className="data-[state=checked]:bg-gradient-to-r data-[state=checked]:from-primary data-[state=checked]:to-accent"
                      />
                    </div>
                  </div>
