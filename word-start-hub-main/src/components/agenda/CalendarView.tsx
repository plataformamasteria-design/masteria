...
export function CalendarView({ selectedDate, onSelectDate, refreshTrigger, calendarId, isGeneralCalendar }: CalendarViewProps) {
  ...
  const fetchData = async () => {
    try {
      ...
      // Calculate visible month range (with padding for days from adjacent months)
      const monthStart = subDays(startOfMonth(selectedDate), 7);
      const monthEnd = addDays(endOfMonth(selectedDate), 7);

      // Fetch all calendars to get colors
      ...
      // Set selected calendar color
      if (calendarId && calMap[calendarId]) {
        setSelectedCalendarColor(calMap[calendarId].color);
      }

      // Fetch events - filter by calendar and date range
      let eventsQuery = supabase
        .from('calendar_events')
        .select('id, start_time, end_time, title, calendar_id')
        .eq('organization_id', currentOrganization.id)
      ...
      // Fetch tasks with due dates - filter by month range
      const { data: tasksData, error: tasksError } = await supabase
        .from('tasks')
        .select('id, due_date, due_time, title, completed')
        .eq('organization_id', currentOrganization.id)
