...
export const ChatWindow: React.FC<ChatWindowProps> = ({ chat, onBack, onMessageSent, onChatDeleted }) => {
  ...
  // Per-channel connection status: check the specific whatsapp_connection for the chat's channel
  const chatSpecificConnected = useMemo(() => {
    const channel = (localChat as any)?.channel;
    if (!channel) return whatsAppStatus.connected;
    if (channel === 'facebook' || channel === 'instagram') return true;
    const conn = connections.find(c => c.instance_name === channel);
    if (conn) return conn.status === 'open';
    return whatsAppStatus.connected;
  }, [localChat, connections, whatsAppStatus.connected]);
  ...
  return (
    <div className="h-full flex flex-col bg-background">
      ...
      <MessageInput
        ...
        isWhatsAppConnected={chatSpecificConnected}
