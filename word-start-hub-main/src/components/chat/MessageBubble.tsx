...
import { ChevronDown, Copy, Download, ExternalLink, Eye, EyeOff, FileText, FolderOpen, Forward, MapPin, Pencil, Phone, Pin, PinOff, QrCode, Reply, SmilePlus, Trash2, User, ZoomIn } from 'lucide-react';
    ...
    >
      <div
        className={cn(
          ...
          isFromAgent
            ? cn('bg-primary rounded-tr-sm shadow-sm agent-message-bubble', message.is_hidden_from_agents && 'border-2 border-dashed border-white/50 bg-primary/90')
            : cn('bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 border border-slate-200 dark:border-slate-700 rounded-tl-sm shadow-sm lead-message-bubble', message.is_hidden_from_agents && 'border-2 border-dashed border-amber-500/50')
        )}
        style={isFromAgent ? {
        ...
        <div className="flex items-center justify-end gap-1 mt-1">
          {message.is_hidden_from_agents && (
            <span className={cn(
              ...
              isFromAgent ? "text-white/90" : "text-amber-600 dark:text-amber-500"
