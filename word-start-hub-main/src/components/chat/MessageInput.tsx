...
import { AiReplySuggestionButton } from "./AiReplySuggestionButton";
...
export const MessageInput: React.FC<MessageInputProps> = ({
  ...
  // Load user preference for AI reply suggestion
  useEffect(() => {
    if (!user?.id) return;
    (supabase.from('profiles') as any)
      .select('ai_reply_suggestion_enabled')
      .eq('id', user.id)
      .maybeSingle()
      .then(({ data }: { data: any }) => {
        setAiReplyEnabled(data?.ai_reply_suggestion_enabled ?? false);
      })
      ...
      <div className={cn(
        ...
        {!isInternalNote && !isRecording && !hasRecordedAudio && (
          <>
            ...
            {/* Botão de sugestão com I.A — visível apenas quando habilitado nas preferências */}
            {aiReplyEnabled && (
              <AiReplySuggestionButton
                chatId={chatId}
                onSuggestion={(text) => {
                  setMessage(text);
                  // Focus textarea after setting the text
