...
function findTriggerNode(nodes: Node[]): Node | undefined {
...
export function useFlowSimulator({ nodes, edges, automationId, onClose, onHighlightNode, onCorrectionsGenerated }: FlowSimulatorUIProps) {
    ...
    const buildSystemPrompt = useCallback((config: any): string => {
        ...
        if (config.dialogue_mode && config.dialogue_objective) {
            parts.push("\n\n🎯 OBJETIVO DO DIÁLOGO: " + config.dialogue_objective);
            parts.push("Quando o objetivo for alcançado, responda EXATAMENTE com: [OBJETIVO_CONCLUÍDO]");
    ...
    const callAI = useCallback(async (config: any): Promise<{ response: string; tokens: number } | null> => {
        try {
            ...
            // Virtual: use in-memory history with full config
            const { data, error } = await supabase.functions.invoke("automation-executor", {
                body: {
                    simulate_ai_virtual: true,
                    organization_id: currentOrganization?.id,
                    config: {
                        credential_id: config.credential_id || "vitta-openai",
        ...
        switch (nodeType) {
            ...
            case "intent_router": {
                ...
                if (result) {
                    ...
                    const matchingEdge = nextNodes.find((n) => n.handleId === result.intent);
            ...
            case "ai_agent": {
                ...
                if (dialogueMode) {
                    ...
                    if (config.dialogue_objective) {
                        addMessage({ type: "system", content: `🎯 Objetivo: ${config.dialogue_objective}`, nodeId, nodeType });
                    }
                    ...
                    while (turn < maxTurns && !abortRef.current && !objectiveCompleted) {
                        ...
                        if (turn < maxTurns) {
                            ...
                            const timeoutAmount = parseInt(config.timeout_amount || "30");
                    ...
                    const singleTimeout = config.timeout_enabled || false;
                    if (singleTimeout) {
                        const tAmount = parseInt(config.timeout_amount || "30");
