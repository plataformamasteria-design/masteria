...
function LoopNodeComponent({ id, data }: NodeProps) {
  ...
  return (
    <div className="bg-card border-2 border-amber-500/60 rounded-xl shadow-lg min-w-[280px] overflow-hidden group">
      ...
      <div className="px-4 py-3 space-y-3">
        ...
        <div className="space-y-1">
          <Label className="text-[10px] text-muted-foreground">Máx. repetições (0 = ilimitado)</Label>
          <Input
            type="number"
            min={0}
            value={config.max_repetitions ?? 0}
            onChange={(e) => update({ max_repetitions: e.target.value })}
            className="h-8 text-xs nodrag"
            placeholder="0"
          />
        </div>
        ...
        <div className="flex items-center gap-2 p-2 rounded-lg bg-muted/40 text-xs text-muted-foreground">
          ...
          <span>
            Após {restartDelay} {unitLabels[restartUnit] || restartUnit}, o fluxo poderá ser reativado para o lead{(config.max_repetitions && parseInt(config.max_repetitions) > 0) ? ` (máx. ${config.max_repetitions}x)` : ""}
          </span>
        </div>
      </div>
