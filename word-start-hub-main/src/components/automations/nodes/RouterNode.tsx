...
function RouterNodeComponent({ id, data }: NodeProps) {
  ...
  return (
    <div className="bg-card border border-border rounded-xl shadow-lg min-w-[340px] overflow-hidden group">
      ...
      <div className="px-4 py-3 space-y-3">
        ...
        <div className="space-y-3">
          {rules.map((rule, i) => {
            ...
            return (
              <div key={i} className="space-y-1.5 rounded-lg border border-border/50 bg-muted/20 p-2">
        ...
        {rules.map((rule, i) => {
          ...
          return (
            <div key={i} className="flex flex-col items-center">
              <span className="text-[8px] text-muted-foreground font-medium truncate max-w-[60px]">{label}</span>
              <Handle
                type="source"
                position={Position.Bottom}
                id={`route-${i}`}
                className={`!w-3 !h-3 ${colorClass.replace("bg-", "!bg-")} !border-2 !border-background !relative !transform-none !left-auto !right-auto`}
              />
            </div>
          );
        })}
        <div className="flex flex-col items-center">
          <span className="text-[8px] text-muted-foreground font-medium">Outros</span>
          <Handle
            type="source"
            position={Position.Bottom}
            id="fallback"
            className="!w-3 !h-3 !bg-gray-400 !border-2 !border-background !relative !transform-none !left-auto !right-auto"
          />
        </div>
      </div>
    </div>
