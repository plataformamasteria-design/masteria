...
export const useChatList = (filter: ChatFilter = 'all') => {
  ...
  const fetchChats = async (showLoading = true) => {
    ...
    // Fetch whatsapp connections for connection display name mapping
    ...
    if (currentOrganization?.id) {
      ...
      if (connections && connections.length > 1) {
        // Only show tags when there are multiple connections
    ...
    const chatsWithTags = await Promise.all(
      (chatsData || []).map(async (chat) => {
        ...
        // Try to find connection display name from channel field
