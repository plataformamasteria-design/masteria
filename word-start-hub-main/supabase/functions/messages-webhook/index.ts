    ...
    if (method === 'POST') {
      ...
      if (body.file_base64 && !['text', 'contact', 'location', 'pix'].includes(body.message_type)) {
        ...
        try {
          ...
          // Try R2 first
          const r2Url = await uploadToR2(bytes, filePath, mimeType);

          if (r2Url) {
            fileUrl = r2Url;
            if (!fileName) fileName = `file.${fileExt}`;
            console.log(`[messages-webhook] ✅ Base64 file uploaded to R2: ${fileUrl}`);
          } else {
            // Fallback to Supabase Storage
            console.log('[messages-webhook] R2 unavailable, falling back to Supabase Storage');
            ...
            if (uploadError) {
              ...
              fileUrl = publicUrl;
              if (!fileName) fileName = `file.${fileExt}`;
              console.log(`[messages-webhook] Base64 file uploaded to Supabase: ${fileUrl}`);
            }
          }
        } catch (error) {
      ...
      // GHL Sync — only fire for INBOUND messages (from lead, is_from_user=false).
      // For OUTBOUND messages (is_from_user=true / platform-sent), trigger-sent-webhooks
      // is already called above and it invokes ghl-outbound-sync internally.
      // Calling it here too was creating a double-sync for every platform message. [BUG-1 FIX]
      if (!body.private && !body.is_from_user) {
        try {
          console.log(`[messages-webhook] Triggering GHL sync for INBOUND message: ${data.id}, type: ${body.message_type}`);
          const ghlResponse = await supabase.functions.invoke('ghl-outbound-sync', {
            body: {
              type: 'INSERT',
              table: 'messages',
              schema: 'public',
          ...
          if (ghlResponse.error) {
            console.error('[messages-webhook] GHL sync error (non-blocking):', ghlResponse.error);
          } else {
            console.log('[messages-webhook] GHL sync completed successfully');
          }
        } catch (ghlErr) {
          console.error('[messages-webhook] GHL sync invocation error (non-blocking):', ghlErr);
          // Non-blocking: GHL sync failure should never prevent message delivery
        }
      }
