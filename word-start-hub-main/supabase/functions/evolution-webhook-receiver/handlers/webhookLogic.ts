...
export function getFileName(message: any, messageType: string, fallbackName?: string): string {
  ...
  const extensions: Record<string, string> = {
    'audio': 'mp3', // Forcing .mp3 instead of .ogg to bypass HighLevel's file extension blocks. Browsers handle the internal ogg/opus codec via content-type anyway.
    'image': 'jpg',
    'video': 'mp4',
    'document': 'pdf',
    'pdf': 'pdf',
...
export async function getEvolutionConfig(supabase: any, org: any, overrideInstanceName?: string | null): Promise<EvolutionApiConfig | null> {
  const { data: globalConfig } = await supabase
    .from('global_config')
    .select('key, value')
    .in('key', ['evolution_api_url', 'evolution_api_key']);

  let globalEvolutionUrl = globalConfig?.find((c: any) => c.key === 'evolution_api_url')?.value;
  const globalEvolutionKey = globalConfig?.find((c: any) => c.key === 'evolution_api_key')?.value;

  if (globalEvolutionUrl && globalEvolutionUrl.endsWith('/')) {
    globalEvolutionUrl = globalEvolutionUrl.slice(0, -1);
  }
  ...
  if (isValidUrl(org.evolution_api_url)) {
    let orgEvolutionUrl = org?.evolution_webhook_url;
    if (orgEvolutionUrl && orgEvolutionUrl.endsWith('/')) {
      orgEvolutionUrl = orgEvolutionUrl.slice(0, -1);
    }
...
export async function fetchProfilePicture(config: EvolutionApiConfig, phone: string): Promise<string | null> {
  try {
    ...
    if (response.ok) {
      ...
      const officialUrl = data.profilePictureUrl;
      console.log('[evolution-webhook-receiver] [PROFILE] Got picture URL:', officialUrl ? 'yes' : 'no');
      
      if (officialUrl && officialUrl.startsWith('http')) {
        try {
          const picRes = await fetch(officialUrl);
          if (picRes.ok) {
            const arrBuffer = await picRes.arrayBuffer();
            const bytes = new Uint8Array(arrBuffer);
            const cleanPhone = phone.replace(/[^a-zA-Z0-9_-]/g, '');
            const path = `profiles/${config.instanceName}/${cleanPhone}_${Date.now()}.jpg`;
            const persistentUrl = await uploadToR2(bytes, path, 'image/jpeg');
            if (persistentUrl) {
              console.log('[evolution-webhook-receiver] [PROFILE] Successfully uploaded profile picture to R2:', persistentUrl);
              return persistentUrl;
            }
          }
        } catch (e) {
          console.error('[evolution-webhook-receiver] [PROFILE] Error capturing and uploading to R2:', e);
        }
      }
...
export async function fetchGroupInfo(config: EvolutionApiConfig, groupJid: string): Promise<any> {
  try {
    ...
    console.warn('[evolution-webhook-receiver] [fetchGroupInfo] API returned', response.status, 'for', groupJid);
  } catch (err) {
    console.error('[evolution-webhook-receiver] Error fetching group info:', err);
  }

  // Retry once after 1s (API may be rate-limited or temporarily unavailable)
  try {
    ...
    if (response.ok) {
      console.log('[evolution-webhook-receiver] [fetchGroupInfo] Retry succeeded for', groupJid);
      return await response.json();
    }
  } catch (retryErr) {
    console.error('[evolution-webhook-receiver] [fetchGroupInfo] Retry also failed:', retryErr);
  }
...
export async function handleGroupEvent(supabase: any, body: any, instanceParam: string | null, organizationIdParam: string | null) {
  try {
    ...
    if (!existingChat?.id) {
      ...
      // Only update name if we got a valid subject, never fallback to "Grupo" on update
      if (groupInfo?.subject && groupInfo.subject.trim() !== '') {
        updateData.group_name = groupInfo.subject;
        updateData.wa_name = groupInfo.subject;
      }
      
      if (groupPhoto) updateData.group_photo_url = groupPhoto;
      if (size !== null) updateData.participant_count = size;
...
export async function getBase64FromMedia(config: EvolutionApiConfig, waMessage: any, convertToMp4: boolean = false): Promise<any> {
  ...
  try {
    const requestBody = {
      message: (waMessage.message && waMessage.key) ? waMessage : (waMessage.data || waMessage),
      convertToMp4: false, // Forçando falso para evitar crash do ffmpeg na API Evolution
    };
