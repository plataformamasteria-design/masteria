...
export async function maybeSendAutoMessages(params: {
    ...
    if (inBusiness && cfg.welcome_enabled && cfg.welcome_message?.trim()) {
        ...
        if (!hadRecentOutbound && welcomeCooldownOk) {
            // Anti-spam: check if this exact welcome was already sent in last 60s
            // This prevents race conditions where parallel webhook invocations both pass cooldown
            const antiSpamCutoff = new Date(now.getTime() - 60_000).toISOString();
            const { data: recentDup } = await supabase
                .from('messages')
                .select('id')
                .eq('chat_id', chat.id)
                .eq('content', cfg.welcome_message.trim())
                .eq('is_from_user', true)
                .eq('sent_from_platform', true)
                .gte('created_at', antiSpamCutoff)
                .limit(1);

            if (recentDup && recentDup.length > 0) {
                console.log('[evolution-webhook-receiver] [AUTO] Welcome suppressed: identical message sent <60s ago');
    ...
    if (!inBusiness && cfg.away_enabled && cfg.away_message?.trim()) {
        ...
        if (awayCooldownOk) {
            // Anti-spam: check if this exact away message was already sent in last 60s
            const awayAntiSpamCutoff = new Date(now.getTime() - 60_000).toISOString();
            const { data: recentAwayDup } = await supabase
                .from('messages')
                .select('id')
                .eq('chat_id', chat.id)
                .eq('content', cfg.away_message.trim())
                .eq('is_from_user', true)
                .eq('sent_from_platform', true)
                .gte('created_at', awayAntiSpamCutoff)
                .limit(1);

            if (recentAwayDup && recentAwayDup.length > 0) {
                console.log('[evolution-webhook-receiver] [AUTO] Away suppressed: identical message sent <60s ago');
