      ...
      <PagePermissionGuard page="leads">
        <div className="space-y-4 md:space-y-6">
          ...
          {loading && chats.length === 0 ? (
            ...
            <div className="grid gap-3 md:gap-4 grid-cols-1 md:grid-cols-2 max-w-full">
              {chats.map((chat) => (
                <LeadCard key={chat.id} chat={chat} onToggleBot={handleToggleBot} onRemoveTag={handleRemoveTag} onRefresh={refreshChats} onClick={() => setSelectedChatId(chat.id)} onNavigateToChat={() => navigate(`/chat?id=${chat.id}`)} onDelete={handleDeleteClick} />
              ))}
