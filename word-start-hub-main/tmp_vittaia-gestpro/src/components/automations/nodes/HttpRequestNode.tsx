...
function HttpRequestNodeComponent({ id, data }: NodeProps) {
  ...
  const updatePair = (key: string, current: KeyValuePair[], pairId: string, field: "name" | "value", val: string) =>
    updatePairs(key, current.map((p) => (p.id === pairId ? { ...p, [field]: val } : p)));
  ...
  return (
    <div className="bg-card border-2 border-orange-500/60 rounded-xl shadow-lg min-w-[260px] max-w-[340px] overflow-hidden">
      <Handle type="target" position={Position.Top} className="!w-3 !h-3 !bg-orange-500 !border-2 !border-card" />
      <NodeHeader
        nodeId={id}
        ...
        <div className="px-3 pb-3 space-y-3 border-t border-border/40 pt-3 max-h-[500px] overflow-y-auto nowheel nodrag">
          <div className="space-y-1">
            ...
            <Select value={method} onValueChange={(v) => updateConfig({ method: v })}>
              ...
              <SelectContent>{METHODS.map((m) => (<SelectItem key={m} value={m} className="text-[11px]"><span className={methodColor[m]}>{m}</span></SelectItem>))}</SelectContent>
            </Select>
          </div>
          <div className="space-y-1">
          ...
          <div className="space-y-1.5">
            ...
            <Select value={authType} onValueChange={(v) => updateConfig({ auth_type: v, auth_config: {} })}>
              <SelectTrigger className="h-7 text-[11px] nodrag"><SelectValue /></SelectTrigger>
              <SelectContent>{AUTH_TYPES.map((a) => (<SelectItem key={a.value} value={a.value} className="text-[11px]">{a.label}</SelectItem>))}</SelectContent>
            </Select>
          ...
          <div className="space-y-1">
            <Label className="text-[10px] font-semibold text-muted-foreground uppercase">Timeout (segundos)</Label>
            <Input
              type="number"
              min={1}
              max={120}
              ...
              placeholder="30"
            />
            <p className="text-[9px] text-muted-foreground">A request será cancelada após este tempo.</p>
          </div>
        </div>
      )}
      ...
      <Handle type="source" position={Position.Bottom} className="!w-3 !h-3 !bg-orange-500 !border-2 !border-card" />
    </div>
  );
}
