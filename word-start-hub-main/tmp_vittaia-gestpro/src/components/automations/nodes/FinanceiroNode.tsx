    ...
    <div className="bg-card border border-border rounded-xl shadow-lg min-w-[260px] overflow-hidden group">
      ...
      <div className="px-4 py-3 space-y-2">
        ...
        <div><Label className="text-[10px] text-muted-foreground">Categoria</Label>
          <Select value={config.category || "geral"} onValueChange={(v) => update({ category: v })}>
            <SelectTrigger className="h-8 text-xs nodrag mt-1"><SelectValue /></SelectTrigger>
            <SelectContent>
        ...
        <div><Label className="text-[10px] text-muted-foreground">Parcelas</Label><Input value={config.installments || "1"} onChange={(e) => update({ installments: e.target.value })} placeholder="1" className="h-8 text-xs nodrag mt-1" type="number" min="1" max="48" /></div>
      </div>
