...
function AskQuestionNodeComponent({ id, data }: NodeProps) {
  ...
  return (
    <div className="bg-card border border-border rounded-xl shadow-lg min-w-[280px] overflow-hidden group">
      ...
      <div className="flex justify-around px-4 pb-3 flex-wrap gap-y-2">
        {options.map((opt, i) => (
          <div key={i} className="flex flex-col items-center">
            <span className="text-[8px] text-muted-foreground truncate max-w-[60px]">{opt}</span>
            <Handle type="source" position={Position.Bottom} id={`opt-${i}`} className="!w-2.5 !h-2.5 !bg-emerald-500 !border-2 !border-background !relative !transform-none !left-auto !right-auto" />
          </div>
        ))}
        <div className="flex flex-col items-center">
          <span className="text-[8px] text-orange-500 font-medium">Outro</span>
          <Handle type="source" position={Position.Bottom} id="fallback" className="!w-2.5 !h-2.5 !bg-orange-500 !border-2 !border-background !relative !transform-none !left-auto !right-auto" />
        </div>
      </div>
    </div>
  );
