...
function CodeNodeComponent({ id, data }: NodeProps) {
  ...
  return (
    <div className="bg-card border-2 border-purple-500/60 rounded-xl shadow-lg min-w-[260px] max-w-[380px] overflow-hidden">
        ...
        <div className="px-3 pb-3 space-y-3 border-t border-border/40 pt-3 max-h-[500px] overflow-y-auto nowheel nodrag">
          ...
          <div className="space-y-1">
            ...
            <Select value={language} onValueChange={(v) => updateConfig({ language: v, code: config.code === DEFAULT_CODE[language] ? DEFAULT_CODE[v] : config.code })}>
              <SelectTrigger className="h-7 text-[11px] nodrag"><SelectValue /></SelectTrigger>
              <SelectContent>
                {LANGUAGES.map((l) => (<SelectItem key={l.value} value={l.value} className="text-[11px]"><span className={LANG_COLORS[l.value]}>{l.label}</span></SelectItem>))}
              </SelectContent>
            </Select>
          </div>
          <div className="space-y-1">
