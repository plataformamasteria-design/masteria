...
function FollowUpAINodeComponent({ id, data }: NodeProps) {
  ...
  const handleSaveCredential = async () => {
    ...
    try {
      const { data: cred, error } = await (supabase as any)
        .from("ai_agent_credentials")
        .insert({ organization_id: currentOrganization.id, name: newCredName, provider: newCredProvider, api_key: newCredKey })
        .select("id, name, provider")
        .single();
      if (error) throw error;
      setCredentials((prev) => [...prev, cred]);
      updateConfig({ credential_id: cred.id, provider: cred.provider });
      setNewCredName(""); setNewCredKey(""); setShowNewCred(false);
  ...
  return (
    <div className="bg-card border-2 border-blue-500/50 rounded-xl shadow-lg w-[340px] overflow-hidden">
      ...
      <div className="px-3 py-3 space-y-2.5 nodrag nowheel max-h-[520px] overflow-y-auto">
        ...
        {showNewCred && (
          <div className="space-y-1.5 rounded-lg bg-muted/30 p-2 border border-border/50">
        ...
        {/* Stop-by-Tag Integration */}
        <div className="rounded-lg border border-fuchsia-500/20 bg-fuchsia-500/5 p-2 space-y-1.5">
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              type="checkbox"
              checked={config.stop_by_tag ?? false}
              onChange={(e) => updateConfig({ stop_by_tag: e.target.checked })}
              className="accent-fuchsia-500 w-3.5 h-3.5 nodrag"
            />
            <span className="text-[10px] font-semibold text-fuchsia-600 dark:text-fuchsia-400 uppercase">🏷️ Parar por Tag</span>
          </label>
          {config.stop_by_tag && (
            <div>
              <Label className="text-[9px] text-muted-foreground">ID da Tag de bloqueio</Label>
              <Input
                value={config.stop_tag_id || ""}
                onChange={(e) => updateConfig({ stop_tag_id: e.target.value })}
                placeholder="UUID da tag"
                className="h-6 text-[10px] nodrag mt-0.5"
              />
              <p className="text-[9px] text-muted-foreground mt-1">Se o lead já tiver esta tag, o follow-up é cancelado automaticamente.</p>
            </div>
          )}
        </div>
      ...
      <div className="flex justify-around px-4 pb-3 pt-1">
        <div className="flex flex-col items-center gap-0.5">
          <span className="text-[8px] text-emerald-600 dark:text-emerald-400 font-medium">Respondeu</span>
          <Handle type="source" position={Position.Bottom} id="responded"
            className="!w-2.5 !h-2.5 !bg-emerald-500 !border-2 !border-card !relative !transform-none !left-auto !right-auto" />
        </div>
        <div className="flex flex-col items-center gap-0.5">
          <span className="text-[8px] text-orange-600 dark:text-orange-400 font-medium">Não respondeu</span>
          <Handle type="source" position={Position.Bottom} id="not_responded"
            className="!w-2.5 !h-2.5 !bg-orange-500 !border-2 !border-card !relative !transform-none !left-auto !right-auto" />
        </div>
      </div>
    </div>
  );
