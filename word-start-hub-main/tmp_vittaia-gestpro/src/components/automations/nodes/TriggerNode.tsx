...
const TRIGGER_LABELS: Record<string, { label: string; icon: any; description: string }> = {
  ...
  broadcast_campaign: { label: "Disparo Lote (Campaign)", icon: Zap, description: "Dispara a partir da aba de Campanhas/Disparos" },
};
...
function TriggerNodeComponent({ data }: NodeProps) {
  ...
  const webhookUrl = webhookToken
    ? `${SUPABASE_URL}/functions/v1/automation-webhook-trigger?token=${webhookToken}`
    : null;
  ...
  return (
    <div className="bg-card border-2 border-primary rounded-xl shadow-lg min-w-[260px] max-w-[320px] overflow-hidden">
      ...
      <div className="px-4 py-3 space-y-2">
        ...
        {triggerType === "webhook" && webhookUrl && (
          <div className="space-y-1.5 p-2 rounded-lg bg-muted/50 border border-border">
            <div className="flex items-center gap-1.5">
              <Globe className="h-3 w-3 text-primary" />
              <span className="text-[10px] font-semibold text-foreground">URL do Webhook</span>
            </div>
            <div className="flex items-center gap-1">
              <code className="text-[9px] font-mono bg-background px-1.5 py-1 rounded border border-border truncate flex-1 block overflow-hidden">
                {webhookUrl}
              </code>
              <Button
                variant="ghost"
        ...
        {triggerType === "scheduled" && scheduleConfig && (
          <div className="space-y-1 p-2 rounded-lg bg-muted/50 border border-border">
            <div className="flex items-center gap-1.5">
