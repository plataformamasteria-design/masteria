...
export function EventDialog({ open, onOpenChange, selectedDate, event, onEventSaved, calendarId, isGeneralCalendar, calendars = [], defaultChatId, defaultAssignedTo, isRescheduling }: EventDialogProps) {
  ...
  const fetchUsers = async () => {
    ...
    const { data } = await supabase.from('profiles').select('id, full_name, email').eq('organization_id', currentOrganization.id).order('full_name');
    if (data) setUsers(data);
  };

  const fetchLeads = async () => {
    if (!currentOrganization?.id) return;
    const { data } = await supabase.from('chats').select('id, wa_name, phone').eq('organization_id', currentOrganization.id).order('wa_name');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isRescheduling && !formData.title.trim()) return toast({ title: "O título é obrigatório", variant: "destructive" });
    if (isRescheduling && !formData.description.trim()) return toast({ title: "O motivo é obrigatório", variant: "destructive" });
    if (!formData.allDay && formData.endTime <= formData.startTime) return toast({ title: "Término inválido", variant: "destructive" });

    try {
      setLoading(true);
      const { data: { user } } = await supabase.auth.getUser();
      if (!user || !currentOrganization?.id) throw new Error("Não autorizado");
      ...
      const eventData = {
        user_id: user.id, title: formData.title.trim(), description: formData.description.trim() || null,
        location: formData.location.trim() || null, start_time: new Date(baseStart).toISOString(),
        end_time: new Date(baseEnd).toISOString(), all_day: formData.allDay, color: formData.color,
        assigned_to: formData.assignedTo || null, chat_id: formData.chatId || null,
      };

      let finalEvId = event?.id;

      if (event) {
        const { error } = await supabase.from('calendar_events').update({ ...eventData, calendar_id: formData.calendarId || null }).eq('id', event.id);
        ...
        const { data: newEvInfo, error } = await supabase.from('calendar_events').insert([{ ...eventData, organization_id: currentOrganization.id, calendar_id: targetCalendarId }]).select('id').single();
        if (error) throw error;
        finalEvId = newEvInfo?.id;
