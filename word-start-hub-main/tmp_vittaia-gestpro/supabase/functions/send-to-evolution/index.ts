...
async function getEvolutionConfig(supabase: any, organizationId: string): Promise<EvolutionConfig | null> {
  ...
  console.log(`[send-to-evolution] Using Evolution instance: ${instanceName}`);
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (messageId) {
      ...
      const config = await getEvolutionConfig(supabase, body.organization_id);
      if (!config) {
        return new Response(
    ...
    const evolutionConfig = await getEvolutionConfig(supabase, chat.organization_id);
