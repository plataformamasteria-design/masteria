...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (method === 'POST') {
      ...
      // GHL Sync — fire for ALL non-private messages (both platform-sent and inbound)
      // This replaces the broken pg_net DB trigger that lacked auth headers
      if (!body.private) {
        try {
          console.log(`[messages-webhook] Triggering GHL sync for message: ${data.id}, type: ${body.message_type}, direction: ${body.is_from_user ? 'outbound' : 'inbound'}`);
