...
async function getEvolutionConfig(supabase: any, organizationId: string): Promise<EvolutionConfig | null> {
  ...
  if (orgData?.evolution_api_url && orgData?.evolution_api_key && orgData?.instance_name) {
    return {
      apiUrl: orgData.evolution_api_url.replace(/\/$/, ""),
      apiKey: orgData.evolution_api_key,
      instanceName: orgData.instance_name,
    };
  }
  ...
  if (globalConfigs && globalConfigs.length >= 2) {
    ...
    if (apiUrl && apiKey && orgData?.instance_name) {
      return {
...
Deno.serve(async (req) => {
  ...
  try {
      ...
      .select(`
        *,
        lead_follow_up_tracking(
          sequence_id,
          current_step,
          chats(id, phone, wa_name, organization_id)
        )
      `)
      .eq("status", "pending")
    ...
    try {
      ...
      const evolutionConfig = await getEvolutionConfig(supabase, queueItem.organization_id);
