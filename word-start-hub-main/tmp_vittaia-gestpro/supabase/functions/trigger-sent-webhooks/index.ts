...
async function getEvolutionConfig(supabase: any, organizationId: string): Promise<EvolutionConfig | null> {
  ...
  console.log(`[trigger-sent-webhooks] Using Evolution instance: ${instanceName}`);
...
async function sendToEvolutionApi(
  ...
  const evolutionConfig = await getEvolutionConfig(supabase, record.organization_id);
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!record) {
      console.error('[trigger-sent-webhooks] No record or messageId provided');
      return new Response(
    ...
    console.log('[trigger-sent-webhooks] Processing message:', record.id, 'type:', record.message_type);
    ...
    const { data: chat, error: chatError } = await supabase
      .from('chats')
      .select('id, phone, is_group, group_name, group_photo_url, wa_name, wa_photo_url')
    ...
    // ==================== STEP 1: SEND TO EVOLUTION API DIRECTLY ====================
    const evolutionResult = await sendToEvolutionApi(
      supabase,
    ...
    if (!evolutionResult.success) {
      console.error('[trigger-sent-webhooks] Failed to send to Evolution API:', evolutionResult.error);
      // Continue to webhooks even if Evolution fails - they might want to handle it differently
    }
