...
import { getInstanceStatus, createInstance, deleteInstance, deleteInstanceFromEvolution, waitUntilInstanceNotFound, getConnectionData, getPairingCode } from './instanceService.ts';
import { updateInstanceWebhook, getInstanceWebhook } from './webhookService.ts';
import { createWhatsAppGroup, updateGroupParticipants, getGroupInviteCode, updateGroupPicture, fetchGroupInfoFromEvolution, syncGroupParticipantsSnapshot, enrichGroupParticipants } from './groupService.ts';
import { ensureLeadFromPhone, validateWhatsAppNumber, sendTextMessage } from './messagingService.ts';
Deno.serve(async (req) => {
  ...
  try {
    ...
    const defaultWebhookUrl = `${supabaseUrl}/functions/v1/evolution-webhook-receiver?instance=${instanceName}`;
    ...
    switch (action) {
      ...
      case 'reconnect': {
        ...
        // ALWAYS delete existing instance first (if exists)
        console.log('Reconnect: deleting existing instance if any...');
        try {
          await deleteInstanceFromEvolution(evolutionConfig, instanceName);
        } catch (e) {
          console.log('Delete attempt failed (may not exist):', e);
        }

        // Wait until the instance name is actually free (Evolution can take a few seconds to propagate deletions)
        const waitRes = await waitUntilInstanceNotFound(evolutionConfig, instanceName, 15000);
        if (!waitRes.success) {
          return new Response(
            JSON.stringify({
              error: 'Não foi possível excluir a instância existente (nome ainda em uso).',
              details: waitRes.error,
            }),
            { status: 409, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
          );
        }

        // Create new instance with same name (webhook will be configured automatically)
