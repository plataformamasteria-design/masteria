...
export async function handleEvolutionWebhook(req: Request) {
  ...
  try {
    ...
    // Se a mensagem foi enviada pela plataforma há menos de 10s e o ID ainda não foi gravado
    if (fromMe) {
      ...
      try {
        ...
        if (recentPending && recentPending.length > 0) {
          const matchedPending = recentPending[0];
          console.log('[evolution-webhook-receiver] Found recent pending message matching echo. Updating ID and skipping insert:', matchedPending.id);
