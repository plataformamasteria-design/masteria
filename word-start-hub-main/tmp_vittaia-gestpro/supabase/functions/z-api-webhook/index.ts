...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Helper function to upload base64 to R2 (with Supabase Storage fallback)
    const uploadBase64ToStorage = async (base64Data: string, fileName: string, mimeType: string) => {
      try {
        ...
        if (r2Url) {
          console.log('[z-api-webhook] ✅ Uploaded to R2:', r2Url);
          return r2Url;
        }

        // Fallback to Supabase Storage
        console.log('[z-api-webhook] R2 unavailable, falling back to Supabase Storage');
  ...
  try {
    ...
    if (waitingExecs && waitingExecs.length > 0) {
      for (const exec of waitingExecs) {
        ...
        }).catch(e => console.error("[z-api-webhook] Error resuming auto:", e));
      }
    }
    ...
    if (chatStages && chatStages.length > 0) {
      ...
      if (msgAutomations && msgAutomations.length > 0) {
        for (const auto of msgAutomations) {
          if (resumedAutomationIds.has(auto.id)) continue;
          ...
          }).catch(e => console.error("[z-api-webhook] Error triggering message_received:", e));
        }
      }
    }
  } catch (err) {
    console.error("[z-api-webhook] Error processing automations on message:", err);
  }
}
