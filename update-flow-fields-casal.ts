...
async function main() {
    try {
        const flows = await db.query.automationFlows.findMany({
            where: ilike(automationFlows.name, '%Formulario Encontro Casal - Aplicação%')
        });
        ...
        const fieldsToAdd = [
            { name: 'nome', value: '{{body.respondent.answers.Qual seu nome completo?}}' },
            { name: 'telefone', value: '{{body.respondent.answers.Qual seu Whatsapp?}}' },
            ...
            { name: 'parceiro_empresario', value: '{{body.respondent.answers.Seu parceiro(a) também é empresário ou estão envolvidos no negócio?}}' },
            ...
            { name: 'maior_desafio', value: '{{body.respondent.answers.Qual o maior desafio que vocês enfrentam hoje como casal de negócios?}}' },
            { name: 'objetivo', value: '{{body.respondent.answers.Qual o seu principal objetivo no Encontro de Casais de Negócios?}}' },
